...
train_pipeline = [
   ...
   dict(type='PointToMultiViewDepth', downsample=1, grid_config=grid_config),
    ...
    dict(type='LoadOccupancy', ignore_nonvisible=True, move_free_to_ign=move_free_to_ign, fix_void=fix_void),
...
# Optimizer
lr = 1.4e-4 # (2e-4 / 64) * (num_gpus * samples_per_gpu)
...
evaluation = dict(
    interval=2 * num_iters_per_epoch, pipeline=test_pipeline)
# num_epochs * num_iters_per_epoch
log_config = dict(
    interval=50,
    hooks=[
        dict(type='WechatLoggerHook',
                by_epoch=False,                  
                keepalive_config=dict(
                    enable=True,
                    expires=25,
                    miao_code='t4OyXz5',
                    text='程序无响应',
                    option='noems,nophonecall,noemail',
                    interval=50,
                 ),),
        dict(type='TextLoggerHook'),
        dict(type='WandbLoggerHook', 
            init_kwargs=dict(
                project='Occupancy Challenge'
            ),
            log_artifact=False,
...
custom_hooks = [
    ...
    dict(
        ...
        temporal_start_iter= 2 * num_iters_per_epoch,
    ),
]
load_from = 'ckpts/r50_256x705_depth_pretrain.pth'
# evaluation = dict(interval=90, pipeline=test_pipeline)
