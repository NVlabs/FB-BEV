...
model = dict(
    ...
    distill_head=dict(
        type='DistillHead',
        loss=dict(type='CustomMSELoss', loss_weight=1),    
    )
    )
...
train_pipeline = [
   ...
   dict(type='ObjectNameFilter', classes=class_names),
    ...
    dict(
        type='Collect3D', keys=['img_inputs', 'points', 'visible_mask_bev', 'gt_depth', 'gt_bboxes_3d', 'gt_labels_3d',
                               ])
]
    ...
    dict(
        ...
        transforms=[
            dict(
                ...
                with_label=False),
            dict(type='Collect3D', keys=['img_inputs', 'points', 'gt_bev_mask', 'gt_occupancy', 'visible_mask'])
        ])
]
...
load_img_from = 'ckpts/r50_fcos3d_pretrain.pth'
load_from = 'ckpts/lidar_occupancy.pth'
