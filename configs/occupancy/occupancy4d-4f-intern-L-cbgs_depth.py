...
model = dict(
    ...
    img_bev_encoder_backbone=dict(
        ...
        with_cp=use_checkpoint,
        block_strides=[1, 2, 2],
        n_input_channels=numC_Trans * (len(range(*multi_adj_frame_id_cfg))+1),
    ...
    dict(
        ...
        transforms=[
            dict(
                ...
                with_label=False),
            dict(type='Collect3D', keys=[ 'img_inputs', 'gt_bev_mask', 'gt_occupancy', 'visible_mask'])
        ])
]
...
# Optimizer
optimizer = dict(type='AdamW', lr=1.4e-4, weight_decay=5e-2,
...
custom_hooks = [
    dict(
        ...
        init_updates=10560,
        priority='NORMAL',
        resume='/mount/data/lsbevv2/work_dirs/occupancy-intern-L_depth_fp16/epoch_1.pth'
    ),
    dict(
...
load_from = 'ckpts/intern_l_coco_pretrain.pth'
