...
def visualize_sample(nusc: NuScenes,
    ...
    ax1 = ax.scatter(points[0, :], points[1, :], c=colors, s=0.2)
    ...
    for box in boxes_est:
        ...
        if box.score >= conf_th:
            index = round(box.score*100)
            color = newcolors[-1][:3]
            print(index, box.score, color)
...
def lidiar_render(sample_token, data,out_path=None, show_type='both'):
    bbox_gt_list = []
    bbox_pred_list = []
...
def render_sample_data(
    ...
    """
    lidiar_render(sample_toekn, pred_data, out_path=out_path, show_type=show_type)
...
if __name__ == '__main__':
    ...
    # bevformer_results = mmcv.load('/mount/data/lsbevv2/test/bevformer4d-r50-depth-cbgs_t2_init_debug_one_neck/Thu_Feb_23_08_01_48_2023/pts_bbox/results_nusc.json')
    bevformer_results = mmcv.load('test2/bevformer3d-r50-depth-cbgs_init_sparse_clean_e1/Mon_Feb_27_14_42_44_2023/pts_bbox/results_nusc.json')
    # bevformer_d_results = mmcv.load('/mount/data/lsbevv2/test/bevformer4d-r50-depth-cbgs_t2_init_debug_one_neck_w_da_test/Sat_Feb_25_06_42_32_2023/pts_bbox/results_nusc.json')
    # lsbev4d_results = mmcv.load('/mount/data/lsbevv2/test/lsbev4d-r50-depth-cbgs_t2_init_debug_one_neck/Wed_Feb_22_01_50_14_2023/pts_bbox/results_nusc.json')
    sample_token_list = list(bevformer_results['results'].keys())
    for id in range(0, 1000, 5):
        render_sample_data(sample_token_list[id], pred_data=bevformer_results, out_path=str(id)+'_'+ sample_token_list[id]+'_bevformer_', show_type='both')
        # render_sample_data(sample_token_list[id], pred_data=bevformer_d_results, out_path=str(id)+'_'+ sample_token_list[id]+'_bevformer_d_',  show_type='both')
        # render_sample_data(sample_token_list[id], pred_data=lsbev4d_results, out_path=str(id)+'_'+ sample_token_list[id]+'_lsbev4d_',  show_type='only_gt')
        # break

# save_tensor(a[:4], f'{ kwargs["img_metas"][0]["sample_idx"]}_{self.count}.png')